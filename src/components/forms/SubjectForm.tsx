...
const SubjectForm = ({ subject, onClose }: SubjectFormProps) => {
  ...
  // Initialize form data based on subject prop
  const initialFormData = subject ? {
    exam_id: (subject as any).exam_id || '',
    name: { en: subject.name, hi: subject.name_hi },
  } : {
    exam_id: '',
    name: { en: '', hi: '' },
  };
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: SubjectInsert }) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (subject) {
      updateMutation.mutate({ id: subject.id, data: subjectData });
    } else {
